...
packageName = "be.intec.models.entities"
entityPackageName = packageName.replaceAll(".models.entities", ".models.entities")
dataBinderPackageName = packageName.replaceAll(".models.entities", ".models.binders")
exceptionsPackageName = packageName.replaceAll(".models.entities", ".services.exceptions")
...
def generateJavaSourceCode(out, className, fields) {
    ...
    fields.each() {
        ...
        else if (it.type == "java.sql.Blob" || it.type == "Blob")
            out.println "    this.set${it.name.capitalize()}(_${it.name}.getBytes());"
        else
            out.println "    this.${it.name} = ${it.name};"
        out.println "  }"
        out.println ""
